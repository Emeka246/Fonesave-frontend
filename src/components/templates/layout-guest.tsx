   ...
   <div className="grid min-h-svh lg:grid-cols-2">
      ...
      <div className="flex flex-col gap-4 p-4 md:p-7">
        <div className="flex gap-2 justify-between items-center">
          <Link to={ROUTES.HOME} className="flex items-center gap-2 self-center font-medium">
            <AppLogo className="h-8 w-auto" />
          </Link>
          <AuthenticatedOnly requireEmailVerification={false}>
              <Button onClick={logoutHandler} size="sm" className="lg:inline-flex">
                  Log out
                <LogOut />
              </Button>
          </AuthenticatedOnly>
        ...
        <p className="text-xs text-muted-foreground text-center mt-4 max-w-md mx-auto">
          By continuing, you agree to FonSave's <Link to={ROUTES.TERMS_CONDITIONS} className="underline text-black dark:text-white">Terms of Service</Link> and <Link to={ROUTES.PRIVACY_POLICY} className="underline text-black dark:text-white">Privacy Policy</Link>, and to receive periodic emails with updates.
        </p>
          ...
          <div className="flex flex-col gap-2 z-10">
           <Quote className="w-16 h-16 text-zinc-300 dark:text-zinc-800 mb-4" />
            <p className="text-3xl text-zinc-800 dark:text-white font-medium">
              Our team is doing some awesome stuff  #fonsave #facts #security
              </p>
              </div>
        </div>
      </div>
