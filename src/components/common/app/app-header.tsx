...
export function AppHeader() {
  ...
  <React.Fragment key={crumb.href}>
    ...
    <BreadcrumbItem className={index < breadcrumbs.length - 1 ? "hidden md:block" : ""}>
      {crumb.isActive ? (
        <BreadcrumbPage className="text-xs md:text-base font-medium">
          <span className="md:hidden">{crumb.title.length > 15 ? crumb.title.substring(0, 15) + '...' : crumb.title}</span>
          <span className="hidden md:inline">{crumb.title}</span>
        </BreadcrumbPage>
      ) : (
        <BreadcrumbLink asChild className="text-xs md:text-base font-medium">
          <Link to={crumb.href}>
            <span className="md:hidden">{crumb.title.length > 15 ? crumb.title.substring(0, 15) + '...' : crumb.title}</span>
            <span className="hidden md:inline">{crumb.title}</span>
          </Link>
