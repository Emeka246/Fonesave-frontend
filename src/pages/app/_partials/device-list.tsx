...
import DeviceService, {PaginationResponse, DevicePayload } from "@/services/device.service";
import { Smartphone, Eye, ChevronLeft, ChevronRight, ChevronsLeft, ChevronsRight, Filter, Search } from "lucide-react";
...
export function DeviceList({ onDevicesChange }: DeviceListProps) {
    ...
    switch (status) {
      ...
      case "STOLEN":
        return <Badge className="bg-red-100 text-red-800 hover:bg-green-100">Stolen</Badge>;
      case "LOST":
        return <Badge className="bg-orange-100 text-orange-800 hover:bg-green-100">Lost</Badge>;
      case "BLOCKED":
        return <Badge className="bg-yellow-100 text-yellow-800 hover:bg-green-100">Blocked</Badge>;
      case "UNKNOWN":
        return <Badge className="bg-gray-100 text-gray-800 hover:bg-green-100">Unknown</Badge>;
      ...
      return (
        <div className="md:flex items-center justify-between mt-6 space-y-4 md:space-y-0">
          <div className="text-sm text-muted-foreground">
            Showing {((currentPage - 1) * pagination.limit) + 1} to {Math.min(currentPage * pagination.limit, pagination.total)} of {pagination.total} devices
        ...
        <div className="md:flex items-center justify-between gap-2 mb-4">
         ...
         {loading ? (
            <Card>
                ...
                <div className="py-8 text-center text-muted-foreground">Loading devices...</div>
                </CardContent>
            </Card>
          ) : devices.length === 0 ? (
            <Card>
            <CardContent className="text-center py-8">
              ...
              <p className="text-muted-foreground mb-4">Register your first device to get started with FonSave protection.</p>
              </CardContent>
            </Card>
          ) : (
            <div className="space-y-4">
              ...
              {devices.length === 0 ? (
                <Card>
                  <CardContent className="text-center py-8">
                    ...
                    <h3 className="text-lg font-semibold mb-2">
                      {statusFilter === 'all' ? 'No devices found' : `No ${statusFilter.toLowerCase()} devices found`}
                    </h3>
                ...
                devices.map((device: DevicePayload) => (
                  <div key={device.id} className="border rounded-lg p-4 hover:bg-muted/50 transition-colors">
                    <div className="md:flex items-start justify-between">
                      ...
                      <div className="flex  flex-col gap-3 items-end">
                        <div className="flex gap-2 mt-3 md:mt-0">
                          ...
                          {!isAgent() || !device?.registrations[0]?.ownerEmail && (
                      ...
                      {device.registrations[0] && device.registrations[0].payment && (
                        <div className="flex items-center gap-2">
                        {getStatusBadge(device.deviceStatus)}
                        {device.registrations[0].payment?.status === 'SUCCESS' && moment(device.registrations[0].expiryDate).isBefore(moment()) && (
                          <Badge className="px-2 font-semibold" variant="green">
                            Payment Paid
                          </Badge>
                        )}
                        {isAgent() && device.registrations[0].payment?.status === 'PENDING' && (
                          <Badge className="px-2 font-semibold" variant="yellow">
                            Registration Pending
                          </Badge>
                        )}
                        {device.registrations[0].payment?.status === 'FAILED' && moment(device.registrations[0].expiryDate).isBefore(moment()) && (
                          <Badge className="px-2 font-semibold" variant="red">
                            Payment Failed
                          </Badge>
                        )}
                        {!device.registrations?.length || moment(device.registrations[0].expiryDate).isBefore(moment()) ? (
                          <Badge className="px-2 font-semibold" variant="red">
                            Expired
                          </Badge>
                        ) : (
                          <Badge className="px-2" variant="outline">
                            Expiry at: {moment(device.registrations[0].expiryDate).format('D MMMM YYYY')}
                          </Badge>
                        )}
                        </div>
                      )}  
