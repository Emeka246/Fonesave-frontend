...
export default function OnboardingPage({
  ...
  useEffect(() => {
    ...
    if (currentUser && currentUser.isEmailVerified === false && !fromVerification) {
    ...
    try {
      ...
      if (response.data.success) {
        ...
        if (formData.role === "AGENT") {
          toast.info("Welcome to the Agent Dashboard! You can now start registering devices for users.");
        } else {
          toast.info("Welcome to FonSave! You can now register your devices.");
        }
        ...
        const returnUrlAfterOnboarding = sessionStorage.getItem('returnUrlAfterOnboarding')
        ...
        const finalReturnUrl = returnUrlAfterOnboarding || returnUrlAfterVerification || returnUrl
  ...
  return (
      <form onSubmit={handleSubmit} className={cn("flex flex-col gap-7", className)}>
         <div className="flex flex-col items-start gap-2">
        <h1 className="text-3xl font-bold">
          Let’s Get You Started!
        </h1>
        <p className="text-muted-foreground text-sm">
          Please complete your profile to unlock all features and personalize your experience.
        </p>
      </div>
        <div className="grid gap-6">
              ...
              <div className="grid gap-2">
                ...
                <RadioGroup 
                  value={formData.role} 
                  onValueChange={(value) => handleRoleChange(value as "USER" | "AGENT")}
                ...
                >
                  ...
                  >
                    <div className="flex gap-3 items-start">
                      <RadioGroupItem value="AGENT" id="agent-role" className="mt-1" />
                      <div className="grid gap-1.5">
                        <span className="text-sm font-medium">Agent</span>
                        <p className="text-xs text-muted-foreground">
                          Register devices on behalf of other users and earn commission
                        </p>
                      </div>
                    </div>
                  </label>
